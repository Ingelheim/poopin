...
class SocketExample: UIViewController, SRWebSocketDelegate {
    ...
    func initHandshake() {
        ...
        var handshakeTask:NSURLSessionTask = session!.dataTaskWithURL(NSURL(string: endpoint)!, completionHandler: { (data:NSData!, response:NSURLResponse!, error:NSError!) in
            if (!(error != nil)) {
